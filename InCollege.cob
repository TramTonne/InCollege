...
01 WS-COUNTERS.
   ...
   05 WS-BROWSE-COUNT PIC 9(6) VALUE 0.
...
01 WS-MESSAGES.
   ...
   05 WS-JOB-SEARCH-HEADER PIC X(30) VALUE "--- Job Search/Internship Menu ---".
   05 WS-JOB-POST-OPTION PIC X(25) VALUE "1. Post a Job/Internship".
   05 WS-JOB-BROWSE-OPTION PIC X(30) VALUE "2. Browse Jobs/Internships".
   05 WS-JOB-MY-APPS-OPTION PIC X(25) VALUE "3. View My Applications".
   05 WS-JOB-BACK-OPTION PIC X(20) VALUE "4. Back to Main Menu".
   05 WS-JOB-POST-HEADER PIC X(30) VALUE "--- Post a New Job/Internship ---".
   05 WS-JOB-TITLE-PROMPT PIC X(20) VALUE "Enter Job Title: ".
   05 WS-JOB-DESC-PROMPT PIC X(35) VALUE "Enter Description (max 200 chars): ".
   05 WS-JOB-EMPLOYER-PROMPT PIC X(25) VALUE "Enter Employer Name: ".
   05 WS-JOB-LOCATION-PROMPT PIC X(20) VALUE "Enter Location: ".
   05 WS-JOB-SALARY-PROMPT PIC X(45) VALUE "Enter Salary (optional, enter 'NONE' to skip): ".
   05 WS-JOB-POSTED-SUCCESS PIC X(25) VALUE "Job posted successfully!".
   05 WS-JOB-BROWSE-UNDER-CONST PIC X(45) VALUE "Browse Jobs/Internships is under construction.".
01 WS-JOB-LIST.
   05 WS-JOB-LIST-ID OCCURS 100 TIMES PIC 9(6).
   05 WS-JOB-LIST-TITLE OCCURS 100 TIMES PIC X(100).
   05 WS-JOB-LIST-EMPLOYER OCCURS 100 TIMES PIC X(100).
   05 WS-JOB-LIST-LOCATION OCCURS 100 TIMES PIC X(100).
01 WS-REPORT.
   05 WS-APP-COUNT PIC 9(6) VALUE 0.
...
01 TEMP-JOBS-STATUS PIC XX.
01 APPLICATIONS-STATUS PIC XX.
01 MESSAGES-STATUS PIC XX.
PROCEDURE DIVISION.
MAIN-PROGRAM.
...
DISPLAY-POST-LOGIN-OPTIONS.
    ...
    MOVE "7. Search for a job" TO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY "7. Search for a job"
...
JOB-SEARCH-MENU.
    ...
    PERFORM UNTIL WS-JOB-EXIT-FLAG = 'Y' OR WS-EOF-FLAG = 'Y'
        ...
        READ INPUT-FILE INTO WS-JOB-CHOICE
            ...
            NOT AT END
                EVALUATE WS-JOB-CHOICE
                    WHEN "1"
                        PERFORM POST-JOB-PROCESS
                    WHEN "2"
                        PERFORM BROWSE-JOBS-PROCESS
                    WHEN "3"
...
DISPLAY-JOB-SEARCH-OPTIONS.
    MOVE WS-JOB-SEARCH-HEADER TO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY WS-JOB-SEARCH-HEADER
    MOVE "1. " TO OUTPUT-RECORD(1:3)
    MOVE "Post a Job/Internship" TO OUTPUT-RECORD(4:)
    WRITE OUTPUT-RECORD
    DISPLAY "1. Post a Job/Internship"
    MOVE "2. " TO OUTPUT-RECORD(1:3)
    MOVE "Browse Jobs/Internships" TO OUTPUT-RECORD(4:)
    WRITE OUTPUT-RECORD
    DISPLAY "2. Browse Jobs/Internships"
    MOVE "3. " TO OUTPUT-RECORD(1:3)
    MOVE "View My Applications" TO OUTPUT-RECORD(4:)
    WRITE OUTPUT-RECORD
    DISPLAY "3. View My Applications"
    MOVE "4. " TO OUTPUT-RECORD(1:3)
    MOVE "Back to Main Menu" TO OUTPUT-RECORD(4:)
    WRITE OUTPUT-RECORD
...
POST-JOB-PROCESS.
    MOVE WS-JOB-POST-HEADER TO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY WS-JOB-POST-HEADER
    MOVE WS-JOB-TITLE-PROMPT TO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY WS-JOB-TITLE-PROMPT
    ...
    WRITE OUTPUT-RECORD
    DISPLAY WS-JOB-EMPLOYER-PROMPT
    ...
    WRITE OUTPUT-RECORD
    DISPLAY WS-JOB-LOCATION-PROMPT
...
BROWSE-JOBS-PROCESS.
    ...
    PERFORM UNTIL WS-EOF-FLAG = 'Y'
        READ JOBS-FILE
            ...
            NOT AT END
                ...
                MOVE JOB-ID TO WS-JOB-LIST-ID(WS-BROWSE-COUNT)
                MOVE JOB-TITLE TO WS-JOB-LIST-TITLE(WS-BROWSE-COUNT)
                MOVE JOB-EMPLOYER TO WS-JOB-LIST-EMPLOYER(WS-BROWSE-COUNT)
                MOVE JOB-LOCATION TO WS-JOB-LIST-LOCATION(WS-BROWSE-COUNT)
                MOVE WS-BROWSE-COUNT TO WS-NUM-BUF
                MOVE SPACES TO OUTPUT-RECORD
                STRING FUNCTION TRIM(WS-NUM-BUF) ". "
                       FUNCTION TRIM(JOB-TITLE) " at " FUNCTION TRIM(JOB-EMPLOYER)
                       " (" FUNCTION TRIM(JOB-LOCATION) ")"
                       DELIMITED BY SIZE INTO OUTPUT-RECORD
                WRITE OUTPUT-RECORD
                DISPLAY WS-BROWSE-COUNT ". " FUNCTION TRIM(JOB-TITLE) " at " FUNCTION TRIM(JOB-EMPLOYER) " (" FUNCTION TRIM(JOB-LOCATION) ")"
        END-READ
    END-PERFORM
    CLOSE JOBS-FILE
    MOVE "-----------------------------" TO OUTPUT-RECORD
    ...
    READ INPUT-FILE INTO WS-TEMP-INPUT
        AT END
            ...
            EXIT PARAGRAPH
    END-READ
    MOVE FUNCTION NUMVAL(FUNCTION TRIM(WS-TEMP-INPUT)) TO WS-SELECT-INDEX
    IF WS-SELECT-INDEX = 0 OR WS-SELECT-INDEX > WS-BROWSE-COUNT
        EXIT PARAGRAPH
    END-IF
    MOVE WS-JOB-LIST-ID(WS-SELECT-INDEX) TO WS-DETAIL-JOB-ID
...
VIEW-JOB-DETAILS.
    ...
    DISPLAY "--- Job Details ---"
    MOVE SPACES TO OUTPUT-RECORD
    STRING "Title: " FUNCTION TRIM(WS-DETAIL-TITLE) DELIMITED BY SIZE INTO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY "Title: " FUNCTION TRIM(WS-DETAIL-TITLE)
    MOVE SPACES TO OUTPUT-RECORD
    STRING "Description: " FUNCTION TRIM(WS-DETAIL-DESC) DELIMITED BY SIZE INTO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY "Description: " FUNCTION TRIM(WS-DETAIL-DESC)
    MOVE SPACES TO OUTPUT-RECORD
    STRING "Employer: " FUNCTION TRIM(WS-DETAIL-EMPLOYER) DELIMITED BY SIZE INTO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY "Employer: " FUNCTION TRIM(WS-DETAIL-EMPLOYER)
    MOVE SPACES TO OUTPUT-RECORD
    STRING "Location: " FUNCTION TRIM(WS-DETAIL-LOCATION) DELIMITED BY SIZE INTO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY "Location: " FUNCTION TRIM(WS-DETAIL-LOCATION)
    IF FUNCTION TRIM(WS-DETAIL-SALARY) NOT = SPACES
        MOVE SPACES TO OUTPUT-RECORD
        STRING "Salary: " FUNCTION TRIM(WS-DETAIL-SALARY) DELIMITED BY SIZE INTO OUTPUT-RECORD
        WRITE OUTPUT-RECORD
        DISPLAY "Salary: " FUNCTION TRIM(WS-DETAIL-SALARY)
    END-IF
    MOVE "-------------------" TO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY "-------------------"
    ...
    WRITE OUTPUT-RECORD
    DISPLAY WS-CHOICE-PROMPT
    READ INPUT-FILE INTO WS-DETAIL-CHOICE
        AT END
            MOVE 'Y' TO WS-EOF-FLAG
            EXIT PARAGRAPH
    END-READ
...
APPLY-FOR-JOB.
    ...
    MOVE FUNCTION CURRENT-DATE(1:8) TO APP-TIMESTAMP(1:8)
    MOVE FUNCTION CURRENT-DATE(9:6) TO APP-TIMESTAMP(9:6)
    MOVE ":" TO APP-TIMESTAMP(11:1)
    MOVE ":" TO APP-TIMESTAMP(14:1)
    WRITE APPLICATION-RECORD
    CLOSE APPLICATIONS-FILE
    MOVE SPACES TO OUTPUT-RECORD
    STRING "Your application for " FUNCTION TRIM(WS-DETAIL-TITLE)
           " at " FUNCTION TRIM(WS-DETAIL-EMPLOYER) " has been submitted."
           DELIMITED BY SIZE INTO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
...
VIEW-MY-APPLICATIONS.
    ...
    MOVE SPACES TO OUTPUT-RECORD
    STRING "Application Summary for " FUNCTION TRIM(WS-USERNAME)
           DELIMITED BY SIZE INTO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY "Application Summary for " FUNCTION TRIM(WS-USERNAME)
    MOVE "------------------------------" TO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY "------------------------------"
                ...
                NOT AT END
                    IF FUNCTION TRIM(APP-USERNAME) = FUNCTION TRIM(WS-USERNAME)
                        ...
                        STRING "Job Title: " FUNCTION TRIM(APP-JOB-TITLE)
                               DELIMITED BY SIZE INTO OUTPUT-RECORD
                        WRITE OUTPUT-RECORD
                        DISPLAY "Job Title: " FUNCTION TRIM(APP-JOB-TITLE)
                        MOVE SPACES TO OUTPUT-RECORD
                        STRING "Employer: " FUNCTION TRIM(APP-JOB-EMPLOYER)
                               DELIMITED BY SIZE INTO OUTPUT-RECORD
                        WRITE OUTPUT-RECORD
                        DISPLAY "Employer: " FUNCTION TRIM(APP-JOB-EMPLOYER)
                        MOVE SPACES TO OUTPUT-RECORD
                        STRING "Location: " FUNCTION TRIM(APP-JOB-LOCATION)
                               DELIMITED BY SIZE INTO OUTPUT-RECORD
                        WRITE OUTPUT-RECORD
                        DISPLAY "Location: " FUNCTION TRIM(APP-JOB-LOCATION)
                        MOVE "---" TO OUTPUT-RECORD
                        WRITE OUTPUT-RECORD
                        DISPLAY "---"
                    END-IF
    ...
    DISPLAY "------------------------------"
    MOVE SPACES TO OUTPUT-RECORD
    STRING "Total Applications: " WS-APP-COUNT
           DELIMITED BY SIZE INTO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY "Total Applications: " WS-APP-COUNT
    MOVE "------------------------------" TO OUTPUT-RECORD
    WRITE OUTPUT-RECORD
    DISPLAY "------------------------------".
...
SAVE-JOB-POSTING.
    ...
    MOVE FUNCTION CURRENT-DATE(1:8) TO JOB-TIMESTAMP(1:8)
    MOVE FUNCTION CURRENT-DATE(9:6) TO JOB-TIMESTAMP(9:6)
    MOVE ":" TO JOB-TIMESTAMP(11:1)
    MOVE ":" TO JOB-TIMESTAMP(14:1)
...
MESSAGES-MENU.
    ...
    PERFORM UNTIL WS-MSG-EXIT-FLAG = 'Y' OR WS-EOF-FLAG = 'Y'
        PERFORM DISPLAY-MESSAGES-OPTIONS
...
SEND-MESSAGE-PROCESS.
    ...
    READ INPUT-FILE INTO WS-MSG-RECIPIENT
        AT END
            ...
            EXIT PARAGRAPH
    END-READ
    MOVE FUNCTION TRIM(WS-MSG-RECIPIENT) TO WS-MSG-RECIPIENT
    ...
    *> Check if they are connected
    ...
    IF WS-IS-CONNECTED = 'N'
        MOVE "You can only send messages to users you are connected with!" TO OUTPUT-RECORD
        WRITE OUTPUT-RECORD
        DISPLAY "You can only send messages to users you are connected with!"
        EXIT PARAGRAPH
    END-IF
...
CHECK-USER-EXISTS.
    ...
    PERFORM UNTIL WS-INDEX > WS-ACCOUNT-COUNT
               OR WS-ACCOUNT-EXISTS = 'Y'
        IF FUNCTION TRIM(WS-MSG-RECIPIENT) = FUNCTION TRIM(WS-STORED-USERNAME(WS-INDEX))
            MOVE 'Y' TO WS-ACCOUNT-EXISTS
        END-IF
        ADD 1 TO WS-INDEX
    END-PERFORM.
...
CHECK-IF-CONNECTED.
    ...
    MOVE 'N' TO WS-EOF-FLAG
    PERFORM UNTIL WS-EOF-FLAG = 'Y' OR WS-IS-CONNECTED = 'Y'
        READ CONNECTIONS-FILE
            AT END
                MOVE 'Y' TO WS-EOF-FLAG
            NOT AT END
                IF FUNCTION TRIM(CONN-STATUS) = "ACCEPTED" AND
                   ((FUNCTION TRIM(CONN-SENDER-USERNAME) = FUNCTION TRIM(WS-USERNAME) AND
                     FUNCTION TRIM(CONN-RECIPIENT-USERNAME) = FUNCTION TRIM(WS-MSG-RECIPIENT)) OR
                    (FUNCTION TRIM(CONN-SENDER-USERNAME) = FUNCTION TRIM(WS-MSG-RECIPIENT) AND
                     FUNCTION TRIM(CONN-RECIPIENT-USERNAME) = FUNCTION TRIM(WS-USERNAME)))
...
SAVE-MESSAGE.
    ...
    MOVE FUNCTION TRIM(WS-MSG-RECIPIENT) TO MSG-RECIPIENT-USERNAME
    MOVE WS-MSG-CONTENT TO MSG-CONTENT
    MOVE FUNCTION CURRENT-DATE(1:8) TO MSG-TIMESTAMP(1:8)
    MOVE FUNCTION CURRENT-DATE(9:6) TO MSG-TIMESTAMP(9:6)
    MOVE ":" TO MSG-TIMESTAMP(11:1)
    MOVE ":" TO MSG-TIMESTAMP(14:1)
